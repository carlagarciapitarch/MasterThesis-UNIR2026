...
QUERY_SMILES = "COc1ccc([C@H]2c3[nH]c4ccc(Cl)cc4c3CCN2C(=O)Oc2ccc(Cl)cc2)cc1"
OUT_DIR = Path("enriched_bits")
GRID_COLS = 4
SUB_IMG_SIZE = (300, 300)
MAX_BITS_TO_DRAW = None  # None = todos los bits relevantes; usa un entero si la figura sale enorme
...
def write_svg(path, svg_text):
    """Guarda un SVG devuelto por RDKit."""
...
def main():
    ...
    # ------------------------------------------------------------------
    # FIGURA ÚNICA CON TODOS LOS BITS RELEVANTES
    # ------------------------------------------------------------------
    bits_to_draw = relevant_bits if MAX_BITS_TO_DRAW is None else relevant_bits[:MAX_BITS_TO_DRAW]

    if bits_to_draw:
        # DrawMorganBits espera tuplas: (mol, bitId, bitInfo).
        tpls = [(query_mol, bit, query_bit_info) for bit in bits_to_draw]

        # Añadimos el enrichment como leyenda para que la figura sea interpretable.
        enrichment_by_bit = df_bits.set_index("bit")["enrichment"].to_dict()
        legends = [
            f"bit {bit} | enrich={enrichment_by_bit.get(bit, float('nan')):.2f}"
        ...
        combined_svg = OUT_DIR / "enriched_bits_relevant_grid.svg"
        write_svg(combined_svg, svg)
        print(f"Figura única guardada: {combined_svg}")
    else:
        print("No hay bits relevantes para dibujar en la molécula candidata.")

    # Opcional: guardar también imágenes individuales de los primeros 10 bits.
    for bit in relevant_bits[:10]:
        png = DrawMorganBit(
            ...
            query_bit_info,
            useSVG=False,
            molSize=SUB_IMG_SIZE,
        )
        out_file = OUT_DIR / f"enriched_bit_{bit}.png"
        write_png(out_file, png)
        print(f"Imagen individual guardada: {out_file}")
